...
function draw(t, e) {
    ...
    tcolour = "random"
    if ($("input#colour").val() != ""){tcolour = $("input#colour").val()}
    vis.selectAll("text").style("fill", tcolour)
}
...
}), form.selectAll("input[type=radio], #font, #colour").on("change", generate);
var stopWords = /^(kaiserplatz)$/, punctuation = new RegExp("[" + unicodePunctuationRe + "]", "g"), wordSeparators = /[ \f\n\r\t\v\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u2028\u2029\u202f\u205f\u3000\u3031-\u3035\u309b\u309c\u30a0\u30fc\uff70]+/g, discard = /^(@|https?:|\/\/)/, htmlTags = /(<[^>]*?>|<script.*?<\/script>|<style.*?<\/style>|<head.*?><\/head>)/g, matchTwitter = /^https?:\/\/([^\.]*\.)?twitter\.com/;
...
();
if ($("#angles > svg").length == 2) {$("#angles > svg")[0].remove()}
